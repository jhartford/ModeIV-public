...
def print_perf(name, oos_perf, filename, args):
    print(f"{args.seed},{name},{args.n_train},{oos_perf},{args.epochs},{args.n_valid},{args.beta}",
          file=open(filename, "a"))


def perf_mode(dataset, models, filename, args, name=''):
    oos_perf = dataset.evaluate(
        lambda x, z, t: models(t, x, z).detach().cpu().numpy())
    print(f"Modal{'' if len(name)==0 else '-'+name} - Out of sample performance evaluated against the true function: {oos_perf}.")
    print_perf(f"modal{'' if len(name)==0 else '-'+name}",
               oos_perf, filename, args)


def perf_deepiv(dataset, filename, args, valid=None, name='', path=None, device='cuda'):
    tensors = [t.detach().cpu().numpy() for t in dataset.dataset.tensors]
    ...
    if os.path.exists(path / f'treatment-{name}.pt'):
        deepiv.treatment_net.load_state_dict(
            torch.load(path / f'treatment-{name}.pt')())
        deepiv.response_net.load_state_dict(
            torch.load(path / f'response-{name}.pt')())
        if '[' in name:
            return deepiv
    else:
        deepiv.fit(epochs_response=args.epochs,
                   epochs_treatment=args.epochs, batch_size=args.batch_size)
        torch.save(deepiv.treatment_net.state_dict,
                   path / f'treatment-{name}.pt')
        torch.save(deepiv.response_net.state_dict,
                   path / f'response-{name}.pt')
    ...
    print_perf(f"deepiv{'' if len(name)==0 else '-'+name}",
               oos_perf, filename, args)
    return deepiv
...
def main():
    ...
    if args.model_id is None or args.model_id == 0:
        perf_deepiv(dataset, filename, args, valid,
                    name='valid', path=path, device=device)
        perf_deepiv(dataset, filename, args, np.arange(
            args.n_inst), name='all', path=path, device=device)
    models = {}
    if args.model_id is None:
        for i in range(args.n_inst):
            models[i] = perf_deepiv(dataset, filename + '-individuals',
                                    args, [i], name=f'marginal-[{i}]', path=path, device=device)
    else:
        print(f"Running model {args.model_id}")
        models[args.model_id] = perf_deepiv(dataset, filename + '-individuals', args, [args.model_id],
                                            name=f'marginal-[{args.model_id}]', path=path, device=device)
